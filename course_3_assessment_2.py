# List comprehension: [<expression> <input sequence> <conditional>]
...
# 6. Below, we have provided a list of tuples that contain students’ names and their final grades in PYTHON 101. Using list comprehension, create a new list passed that contains the names of students who passed the class (had a final grade of 70 or greater).
...
#  7. Write code using zip and filter so that these lists (l1 and l2) are combined into one big list and assigned to the variable opposites if they are both longer than 3 characters each.


l1 = ['left', 'up', 'front']
l2 = ['right', 'down', 'back']

opposites = filter(lambda x: len(x[0]) > 3 and len(x[1]), zip(l1, l2) )

# 8. Below, we have provided a species list and a population list. Use zip to combine these lists into one list of tuples called pop_info. From this list, create a new list called endangered that contains the names of species whose populations are below 2500.
...
# BONUS Remember below for dictionaries - search through all the values in the dict, then for item['name'] key for each item in x
[ [item['name'] for item in x ] for x in dict.values()[0]
